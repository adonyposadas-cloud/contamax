...
;(function () {
  ...
  window.initComisionesDash = async function () {
    ...
    if (!root) return
    if (!esAutorizado()) { root.innerHTML = '<div style="padding:24px;color:var(--red-fg,#f85149)">Solo gerencia puede ver las comisiones.</div>'; return }
    root.innerHTML = '<div style="padding:24px;color:var(--text2,#8b949e)">Cargando comisiones…</div>'
    try {
  ...
  function render (filas) {
    ...
    root.innerHTML = `
      ...
      ${tecnicos.length ? `
      <div style="border:1px solid var(--border,#2a2e37);border-radius:12px;overflow:hidden">
        <div style="display:flex;padding:9px 14px;background:var(--bg2,#15171c);font-size:11px;color:var(--text2,#8b949e);font-weight:700;letter-spacing:.04em">
      ...
      </div>` : '<div style="padding:30px;text-align:center;color:var(--text2,#8b949e)">No hay comisiones en este período. La comisión corre cuando la orden se finaliza.</div>'}
  ...
  window.comDashDetalle = async function (tecnicoId) {
    ...
    cont.innerHTML = '<div style="padding:14px;color:var(--text2,#8b949e)">Cargando detalle…</div>'

    const { data, error } = await sb().from('v_checklist_comision').select('*').eq('tecnico_id', tecnicoId).limit(500)
    if (error) { cont.innerHTML = `<div style="padding:14px;color:var(--red-fg,#f85149)">${esc(error.message)}</div>`; return }
