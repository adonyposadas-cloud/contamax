...
//  · Una partida se puede exportar solo si sus líneas fiscales cuadran.
...
(function () {
  ...
  function analizar(p, lineas) {
    ...
    else if (Math.abs(deb - cre) > 0.005) estado = 'descuadre'
    return { ...p, lineas, fiscales, deb, cre, estado, parcial: fiscales.length > 0 && fiscales.length < lineas.length }
  }
  ...
  // Al cambiar "excluir centros privados" se recalcula sin volver a consultar.
  window.efReanalizar = function () {
    if (!efPartidas.length) return
    efPartidas = efPartidas.map(p => analizar(p, p.lineas))
    efPartidas.forEach(p => { if (p.estado !== 'ok') efSel.delete(p.id) })
    efRender()
  }
  ...
  function efRender() {
    ...
    const totalSel = r2(selArr.reduce((s, p) => s + p.deb, 0))
  ...
  function filaHTML(p) {
    ...
    const nota = p.estado === 'descuadre' ? `<div style="font-size:11px;color:var(--red-fg)">Diferencia L. ${fmt(dif)}</div>`
    ...
    return `
      <tr style="cursor:pointer" onclick="efToggleDetalle('${p.id}')">
        ...
        <td>${esc(p.descripcion || '')}${fotos}</td>
        <td style="text-align:right">${fmt(p.deb)}</td>
        <td style="text-align:right">${fmt(p.cre)}</td>
  ...
  function detalleHTML(p) {
    ...
    return `<tr><td></td><td colspan="7" style="background:var(--bg3)">
      ...
      <tbody>${filas || '<tr><td colspan="5">Sin líneas</td></tr>'}</tbody></table>
      ${p.estado === 'descuadre' ? '<div style="font-size:12px;color:var(--text2);margin-top:6px">Corrige la casilla <b>fiscal</b> de las líneas en la partida para que lo que va al fiscal cuadre, y vuelve a cargar.</div>' : ''}
    </td></tr>`
  }
    ...
    try {
        ...
        const lineas = p.fiscales
          ...
          .map(l => {
            ...
            return {
              ...
              descripcion: l.descripcion || null, numero_documento: l.numero_documento || null,
            }
          })
        const cuerpo = {
          fecha: p.fecha_partida, numero_documento: p.numero_documento || null,
          descripcion: p.descripcion || '(sin descripción)', tipo_origen: p.tipo_origen || null, lineas,
        }
      ...
      for (const v of efLibros.ventas) {
        ...
        libro_ventas.push({
          ...
          incluir_fiscal: v.incluir_fiscal !== false, numero_documento: v.numero_documento, observaciones: v.observaciones,
        })
      }
      ...
      for (const c of efLibros.compras) {
        ...
        libro_compras.push({
          ...
          productos: c.productos, tipo_compra: c.tipo_compra, incluir_fiscal: c.incluir_fiscal !== false,
